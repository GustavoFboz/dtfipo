...
import { Plus, ChevronRight, Filter, X, Trash2, CalendarDays, FileDown } from "lucide-react";
...
function Index() {
    ...
    const [caseYear, setCaseYear] = useState(null);
    const [counts, setCounts] = useState({ all: 0, em_andamento: 0, atrasados: 0, finalizados: 0, arquivados: 0 });
    const [dateRange, setDateRange] = useState(null);
    const [advancedFilters, setAdvancedFilters] = useState({ doctorIds: [], cadistaIds: [] });
    ...
    useEffect(() => {
        window.DENTALFLOW_TRASH_MODE = isTrashMode;
        return () => { window.DENTALFLOW_TRASH_MODE = false; };
    }, [isTrashMode]);
    useEffect(() => {
        try {
            ...
            const filtered = allCases.filter((c) => {
                ...
                if (currentActiveFilter !== "all") {
                    if (currentActiveFilter === "deleted") {
                        ...
                        if (c.finished || c.status === "finalizado" || c.status === "arquivado" || c.status === "cancelado")
                            return false;
                    }
                        ...
                        if (c.finished || c.status === "finalizado" || c.status === "arquivado" || c.status === "cancelado")
                            return false;
                        if (!isLate(c.delivery_date))
                            return false;
                    }
                    else if (currentActiveFilter === "finalizados") {
                        if (!c.finished && c.status !== "finalizado")
                            return false;
                    }
                ...
                if (dateRange && (dateRange.start || dateRange.end)) {
                    ...
                    try {
                        ...
                        if (dateRange.start) {
                            ...
                            if (!isNaN(startDate.getTime()) && caseDate < startDate)
                                return false;
                        }
                        if (dateRange.end) {
                            const endDate = new Date(dateRange.end + "T00:00:00");
                            if (!isNaN(endDate.getTime()) && caseDate > endDate)
                                return false;
                        }
                    }
                    catch {
                ...
                if (advancedFilters.doctorIds && advancedFilters.doctorIds.length > 0) {
                    if (!c.doctor_id || !advancedFilters.doctorIds.includes(c.doctor_id))
                        return false;
                }
                if (advancedFilters.cadistaIds && advancedFilters.cadistaIds.length > 0) {
                    if (!c.cadista_id || !advancedFilters.cadistaIds.includes(c.cadista_id))
                        return false;
                }
                return true;
            });
    ...
    return (<div className={`h-full max-h-full overflow-hidden flex flex-col font-light max-w-[1600px] mx-auto w-full px-6 md:px-16 transition-colors duration-500 ${isTrashMode ? "bg-rose-50/30 dark:bg-rose-950/10" : ""} ${exiting ? "animate-lab-exit" : entering ? "animate-lab-enter" : ""}`}>
        ...
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-8">
          <div className="relative h-20 md:h-24 xl:h-28 flex items-center overflow-hidden">
            <AnimatePresence mode="wait">
              {!isTrashMode ? (<motion.h1 key="normal-title" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -40, opacity: 0 }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} className="text-4xl lg:text-5xl xl:text-7xl font-extralight text-slate-900 dark:text-slate-100 tracking-[-0.03em] leading-[1] flex flex-wrap items-baseline gap-2 md:gap-4">
                  ...
                  {(caseYear != null || dateRange != null) && (<span className="text-primary whitespace-nowrap">
                      {dateRange ? (dateRange.start && dateRange.end && dateRange.start.split('-')[0] !== dateRange.end.split('-')[0]
                    ? `${dateRange.start.split('-')[0]}-${dateRange.end.split('-')[0]}`
                    : dateRange.start.split('-')[0]) : caseYear}
                    </span>)}
                  {caseYear == null && dateRange == null && (<span className="text-primary whitespace-nowrap">{new Date().getFullYear()}</span>)}
                </motion.h1>) : (<motion.h1 key="trash-title" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -40, opacity: 0 }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} className="text-4xl lg:text-5xl xl:text-7xl font-extralight text-rose-600 dark:text-rose-400 tracking-[-0.03em] leading-[1] flex flex-wrap items-baseline gap-2 md:gap-4">
                  ...
                  {(caseYear != null || dateRange != null) && (<span className="text-rose-400 whitespace-nowrap">
                      {dateRange ? (dateRange.start && dateRange.end && dateRange.start.split('-')[0] !== dateRange.end.split('-')[0]
                    ? `${dateRange.start.split('-')[0]}-${dateRange.end.split('-')[0]}`
                    : dateRange.start.split('-')[0]) : caseYear}
                    </span>)}
                  {caseYear == null && dateRange == null && (<span className="text-rose-400 whitespace-nowrap">{new Date().getFullYear()}</span>)}
                </motion.h1>)}
            </AnimatePresence>
          </div>

          <div className="flex items-center gap-4 w-full lg:w-auto lg:min-w-[300px] justify-end">
            <PatientFormDialog open={openNewPatient} onOpenChange={setOpenNewPatient} trigger={<Button variant="ghost" className="h-14 px-8 rounded-full bg-white hover:bg-slate-50 text-slate-400 border border-slate-100 shadow-sm font-normal text-[15px] gap-2 transition-all hover:-translate-y-[1px]">
                  <Plus className="h-4 w-4 stroke-[1.5px]"/> Novo paciente
                </Button>}/>
            <NewCaseDialog trigger={<Button className="h-14 px-8 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/25 font-normal text-[15px] gap-2 transition-all hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-[1px]">
      ...
      <section className="flex-1 min-h-0 flex flex-col">
          ...
          <AnimatePresence mode="wait">
            {true && (<motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className={`flex flex-wrap items-center gap-2 p-1 rounded-[2rem] w-fit transition-colors duration-500 ${isTrashMode ? "bg-rose-100/50 dark:bg-rose-900/20" : "bg-slate-100/50 dark:bg-white/5"}`}>
            {[
                { id: "all", label: "Todos" },
                ...
                return (<button key={t.id} onClick={() => setFilter(t.id)} className={`relative flex items-center gap-2 px-6 py-2.5 rounded-full text-[13px] font-medium transition-colors duration-300 whitespace-nowrap z-10 ${isActive
                        ? "text-white"
                        : isTrashMode
                            ? "text-rose-300 hover:text-rose-500"
                            : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"}`}>
                  {isActive && (<motion.div layoutId="active-filter" className={`absolute inset-0 shadow-lg rounded-full -z-10 transition-colors duration-500 ${isTrashMode ? "bg-rose-500 shadow-rose-400/20" : "bg-[#54A8FB] shadow-blue-400/20"}`} transition={{
                            type: "spring",
                            stiffness: 400,
                            damping: 30,
                        }}/>)}
                  ...
                  <AnimatePresence mode="popLayout">
                    {isActive && (<motion.span key={`count-${t.id}`} initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} transition={{
                            type: "spring",
                            stiffness: 600,
                            damping: 25,
          ...
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={handleDownloadReport} className="h-11 w-11 rounded-full border border-slate-200/50 dark:border-white/5 bg-white dark:bg-white/5 text-slate-400 hover:text-primary transition-all shadow-sm" title="Gerar Relatório PDF">
              <FileDown className="h-4 w-4 stroke-[1.5px]"/>
            </Button>

            <AnimatePresence mode="wait">
              {true && (<motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className={`flex items-center gap-2 p-1 rounded-full border transition-all duration-500 ${isTrashMode ? "bg-rose-100/50 dark:bg-rose-900/20 border-rose-200/50" : "bg-slate-100/50 dark:bg-white/5 border-slate-200/50 dark:border-white/5"}`}>
              <Popover>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <Input type="date" value={localStartDate} onChange={(e) => setLocalStartDate(e.target.value)} className="h-11 rounded-2xl bg-white dark:bg-white/5 border-slate-100 dark:border-white/5 focus-visible:ring-1 focus-visible:ring-[#54A8FB]"/>
                      </div>
                      <div className="space-y-2">
                        <label className="text-[11px] text-slate-400 font-medium px-1">FIM</label>
                        <Input type="date" value={localEndDate} onChange={(e) => setLocalEndDate(e.target.value)} className="h-11 rounded-2xl bg-white dark:bg-white/5 border-slate-100 dark:border-white/5 focus-visible:ring-1 focus-visible:ring-[#54A8FB]"/>
                      </div>
                    </div>
                    <Button onClick={() => setDateRange({ start: localStartDate, end: localEndDate })} className="w-full h-11 rounded-full bg-[#54A8FB] hover:bg-[#4a97e2] text-white shadow-lg shadow-blue-400/20 font-medium">
                      Aplicar Filtro
                    </Button>
                  </div>
                </PopoverContent>
              ...
              {(dateRange || advancedFilters.doctorIds.length > 0 || advancedFilters.cadistaIds.length > 0) && (<>
                  <div className="w-[1px] h-4 bg-slate-200 dark:bg-white/10 mx-1"/>
                  <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full text-slate-400 hover:text-primary transition-all" onClick={() => {
                    setDateRange(null);
                    setLocalStartDate("");
                    setLocalEndDate("");
            ...
            if (!isTrashMode)
                ...
                setFilter("all");
        }} className={`h-11 w-11 rounded-full border border-slate-200/50 dark:border-white/5 transition-all shadow-sm ${isTrashMode
            ? "bg-rose-500 text-white hover:bg-rose-600 border-rose-400 shadow-rose-200/50"
            : "bg-white dark:bg-white/5 text-slate-400 hover:text-rose-500"}`} title={isTrashMode ? "Voltar aos Casos" : "Lixeira e Cancelados"}>
        ...
        <CasesTable hideToolbar minimal hideSearch activeFilter={isTrashMode ? "deleted" : filter} onFilterChange={setFilter} onYearChange={setCaseYear} onCountsUpdate={setCounts} dateRange={dateRange} advancedFilters={advancedFilters}/>
      </section>
...
function AdvancedFilterContent({ filters, setFilters }) {
    ...
    return (<div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
      ...
      <div className="space-y-5">
        <div className="space-y-3">
          ...
          <div className="max-h-[120px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
            {stages?.map(s => (<div key={s.id} className="flex items-center gap-2 px-1">
                <Checkbox id={`stage-${s.id}`} checked={selectedStages.includes(s.id)} onCheckedChange={() => toggleStage(s.id)} className="rounded-md border-slate-200 dark:border-white/10"/>
        ...
        <div className="space-y-3">
          ...
          <div className="max-h-[120px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
            {doctors?.map(d => (<div key={d.id} className="flex items-center gap-2 px-1">
                <Checkbox id={`doc-${d.id}`} checked={filters.doctorIds.includes(d.id)} onCheckedChange={() => toggleDoctor(d.id)} className="rounded-md border-slate-200 dark:border-white/10"/>
        ...
        <div className="space-y-3">
          ...
          <div className="max-h-[120px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
            {cadistas?.map(c => (<div key={c.id} className="flex items-center gap-2 px-1">
                <Checkbox id={`cad-${c.id}`} checked={filters.cadistaIds.includes(c.id)} onCheckedChange={() => toggleCadista(c.id)} className="rounded-md border-slate-200 dark:border-white/10"/>
