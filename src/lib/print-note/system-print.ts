...
function qrPayload(caseRow: CaseRow): string {
  ...
  try {
    const value=`${window.location.origin}/?case=${encodeURIComponent(caseRow.id)}`;
...
function buildReferenceLayout(caseRow: CaseRow, settings: CaseNotePrinterSettings, components: ImplantComponentLine[]): string {
  ...
  const baseScale=Math.max(.34,Math.min(1.75,Math.min(paper.widthMm/100,paper.heightMm/170)));
...
export async function printCaseNoteSystem(caseRow:CaseRow,settings:CaseNotePrinterSettings):Promise<void>{
  ...
  try{
    ...
    await new Promise<void>(resolve=>{if(doc.readyState==="complete")return resolve();iframe.addEventListener("load",()=>resolve(),{once:true});window.setTimeout(resolve,250);});
