...
export function AppShell() {
  ...
  return (
    <div className={`flex h-screen overflow-hidden bg-[#fcfdfe] dark:bg-black font-light transition-all duration-500 ${dialogOpen ? "blur-md" : ""}`}>
      {/* ============ DESKTOP TOP HEADER ============ */}
      <header className={`hidden md:flex fixed top-0 left-0 right-0 z-50 bg-[#F9FAFB] dark:bg-slate-950 border-b border-slate-100 dark:border-white/5 items-center justify-between px-6 transition-all duration-300 ease-in-out h-[72px] ${dialogOpen ? "pointer-events-none grayscale-[0.2]" : ""}`}>
      ...
      <aside
        ...
        } ${isCollapsed ? "w-[80px]" : "w-64"} ${dialogOpen ? "pointer-events-none grayscale-[0.2]" : ""}`}
      >
        <TooltipProvider>
      ...
      <main data-scroll-container="app" className={`relative h-screen flex-1 min-w-0 pt-[72px] md:pt-[72px] overflow-y-auto overflow-x-hidden bg-white dark:bg-slate-950 transition-all duration-300 ${dialogOpen ? "blur-md" : ""}`}>
