...
export function AppShell() {
  ...
  return (
    <div className={`flex h-screen overflow-hidden bg-[#fcfdfe] dark:bg-black font-light transition-all duration-500`}>
      {/* ============ DESKTOP TOP HEADER ============ */}
      <header className={`hidden md:flex fixed top-0 left-0 right-0 z-50 bg-[#F9FAFB] dark:bg-slate-950 border-b border-slate-100 dark:border-white/5 items-center justify-between px-6 transition-all duration-300 ease-in-out h-[72px] ${dialogOpen ? "blur-md pointer-events-none opacity-90 brightness-[0.98] grayscale-[0.1]" : ""}`}>
      ...
      <aside
        ...
        } ${isCollapsed ? "w-[80px]" : "w-64"} ${dialogOpen ? "blur-md pointer-events-none opacity-90 brightness-[0.98] grayscale-[0.1]" : ""}`}
      >
        <TooltipProvider>
        <div className="flex flex-col h-full">
          {profile && (
            <div className={`transition-[padding] duration-300 ease-in-out overflow-hidden ${isCollapsed ? "pt-6 px-5" : "p-6 pt-10"}`}>
              <h2 className={`text-[17px] font-medium text-primary mb-6 transition-opacity duration-300 ease-in-out whitespace-nowrap ${isCollapsed ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
                ...
                <Tooltip key={n.to} delayDuration={0}>
                  <TooltipTrigger asChild>
                    <Link
                      ...
                      className={`group relative flex items-center rounded-xl text-sm transition-[padding,margin,width,background-color] duration-300 ease-in-out overflow-hidden ${
                        active
                    ...
                    >
                      ...
                      {!isCollapsed && (
                        <span className="font-light tracking-wide whitespace-nowrap transition-opacity duration-300 ease-in-out flex-1 overflow-hidden">
                          {n.label}
                        </span>
                      )}
            ...
            <Tooltip delayDuration={0}>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  />
                  <span className={`transition-all duration-300 ease-in-out whitespace-nowrap group-hover:text-primary ${isCollapsed ? "opacity-0 w-0" : "opacity-100 w-auto"}`}>
                    CLÍNICA
                  </span>
                  {isCollapsed && (
      ...
      <main data-scroll-container="app" className={`relative h-screen flex-1 min-w-0 pt-[72px] md:pt-[72px] overflow-y-auto overflow-x-hidden bg-white dark:bg-slate-950 transition-all duration-300 ${dialogOpen ? "blur-md pointer-events-none" : ""}`}>
