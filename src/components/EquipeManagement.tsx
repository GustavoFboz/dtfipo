...
import { CompanyTeamAccessPanel } from "./CompanyTeamAccessPanel";

const ALL_ROLES = ["CEO", "DR", "PROTETICO", "CADISTA", "ATENDIMENTO", "SOLICITANTE", "USER"];
const CLINIC_ROLES = ["CEO", "DR", "ATENDIMENTO", "USER"];
const ROLE_LABEL: Record<string, string> = { CEO: "Administrador", DR: "Dentista", PROTETICO: "Protético", CADISTA: "Cadista", ATENDIMENTO: "Atendimento", SOLICITANTE: "Solicitante", USER: "Usuário" };
...
export function EquipeManagement({ mode = "laboratory" }: { mode?: "laboratory" | "clinic" }) {
  ...
  const canManageCompanyAccess = Boolean(currentProfile?.is_default_admin) || ["CEO", "ADMIN"].includes(effectiveCurrentRole);
  ...
  const deleteMember = useMutation({
    mutationFn: async (user_id: string) => {
      const target = profiles.find((p: any) => p.id === user_id);
      if (target?.is_default_admin || target?.is_founder) throw new Error("O fundador da empresa não pode ser removido.");
    ...
    onSuccess: () => { toast.success("Membro removido com sucesso"); void qc.invalidateQueries({ queryKey: ["profiles_full"] }); void qc.invalidateQueries({ queryKey: ["company_team_access"] }); },
  ...
  if (isError && profiles.length === 0) return <div className="rounded-[28px] border border-amber-200/60 bg-amber-50/40 px-6 py-12 text-center dark:border-amber-500/15 dark:bg-amber-500/[0.04]"><Users2 className="mx-auto h-8 w-8 text-amber-300" /><p className="mt-3 text-sm font-medium text-amber-800 dark:text-amber-300">Não foi possível carregar a equipe agora.</p><p className="mx-auto mt-1 max-w-lg text-xs font-light text-amber-700/70 dark:text-amber-300/60">{(error as Error)?.message || "A sessão será revalidada automaticamente. Tente novamente em instantes."}</p></div>;

  return (
    <div className="space-y-5">
      {isAdmin && <JoinRequestsPanel mode={mode} />}
      {canManageCompanyAccess && <CompanyTeamAccessPanel />}
      ...
      <div className="flex flex-col gap-3 rounded-[24px] border border-slate-200/70 bg-white p-3 md:flex-row md:items-center md:justify-between dark:border-white/10 dark:bg-slate-950">
        <div className="flex flex-wrap gap-1.5"><FilterButton active={roleFilter === "all"} onClick={() => setRoleFilter("all")} label="Todos" clinicMode={clinicMode} />{roleOptions.map((role) => <FilterButton key={role} active={roleFilter === role} onClick={() => setRoleFilter(role)} label={ROLE_LABEL[role]} clinicMode={clinicMode} />)}</div>
...
function FilterButton({ active, onClick, label, clinicMode }: any) { return <button onClick={onClick} className={`h-9 rounded-xl px-3 text-xs font-medium transition ${active ? (clinicMode ? "bg-[#1e8f87]/10 text-[#1e8f87]" : "bg-primary/10 text-primary") : "text-slate-400 hover:bg-slate-50 hover:text-slate-600 dark:hover:bg-white/5"}`}>{label}</button>; }
function Contact({ icon: Icon, value, accent }: any) { return <div className="flex min-w-0 items-center gap-2.5 text-xs font-light text-slate-500"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-slate-50 dark:bg-white/[0.035]"><Icon className="h-3.5 w-3.5" style={accent ? { color: accent } : undefined} /></div><span className="truncate">{value}</span></div>; }
