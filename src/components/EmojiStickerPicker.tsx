...
export function EmojiStickerPicker({
  onPickEmoji, onPickSticker, trigger, inline = false,
}: {
                ...
                <div className="grid grid-cols-8 gap-1">
                  {g.items.map((e) => (
                    <button key={e} type="button"
                      onClick={() => { onPickEmoji(e); if (!inline) setOpen(false); }}
  ...
  if (inline) {
    return <div className="w-80 rounded-xl border bg-popover text-popover-foreground shadow-lg overflow-hidden">{content}</div>;
  }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-80 p-0">{content}</PopoverContent>
    </Popover>
  );
}
