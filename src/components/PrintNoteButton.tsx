...
import { DesktopPrinterSetup } from "@/components/DesktopPrinterSetup";
import {
...
export function PrintNoteButton({ caseRow, variant = "pill" }: { caseRow: CaseRow; variant?: Variant }) {
  ...
  function requestSetup(userId: string, reason?: string, preferred?: CaseNotePrinterSettings | null) {
    setActiveUserId(userId);
    setSettings(preferred ?? loadCaseNotePrinterSettingsForAccount(userId) ?? loadCaseNotePrinterSettings());
  ...
  async function executePrint(userId: string, selected: CaseNotePrinterSettings) {
    ...
    try {
      if (selected.transport === "bluetooth") {
        if (!bluetoothSupported()) throw new Error("A impressora Bluetooth configurada não está disponível neste dispositivo.");
        const tpl = await fetchMyPrintTemplate();
        if (!tpl) throw new Error("A configuração Bluetooth desta conta está incompleta.");
        await printNoteBluetooth(caseRow, tpl);
        return;
      }
      if (desktop) {
        await printCaseNoteDirectDesktop(caseRow, selected);
        return;
      }
      ...
      if (selected.transport === "bluetooth" || desktop || isPrinterAvailabilityError(err)) requestSetup(userId, message, selected);
  ...
  async function handleClick(e: React.MouseEvent) {
    ...
    if (!userId) return void toast.error("Sua sessão expirou. Entre novamente para imprimir a nota.");
    const saved = loadCaseNotePrinterSettingsForAccount(userId);
    if (!saved || (desktop && saved.transport === "system" && !saved.printerName)) {
      requestSetup(userId, "Configure a impressora da Nota do Caso uma única vez neste dispositivo.", saved);
      return;
    }
    try { await executePrint(userId, saved); } catch { /* setup/error handled above */ }
  }

  async function saveAndPrintFromSetup() {
    const userId = activeUserId ?? await currentUserId();
    if (!userId) return void toast.error("Sua sessão expirou. Entre novamente para configurar a impressora.");
    if (desktop && settings.transport === "system" && !settings.printerName?.trim()) {
      toast.error("Selecione uma impressora do Windows.");
      return;
    }
    saveCaseNotePrinterSettingsForAccount(userId, settings);
    setSetupReason(null);
    try { await executePrint(userId, settings); setSetupOpen(false); } catch { /* keep setup open */ }
  }

  return (
    <>
      {variant === "icon" ? (
        <button type="button" disabled={busy} onClick={handleClick} className="h-8 w-8 inline-flex items-center justify-center rounded-md hover:bg-muted text-muted-foreground hover:text-foreground disabled:opacity-60" title="Imprimir nota do caso" aria-label="Imprimir nota do caso"><Printer className="h-4 w-4" /></button>
      ) : (
        <button type="button" disabled={busy} onClick={handleClick} className="h-8 px-3 inline-flex items-center gap-1.5 rounded-full border border-primary/30 bg-background text-primary hover:bg-primary/10 transition text-xs font-medium disabled:opacity-60" title="Imprimir nota do caso" aria-label="Imprimir nota do caso"><Printer className="h-3.5 w-3.5" />{busy ? "Imprimindo…" : "Nota"}</button>
      )}
      ...
      <Dialog open={setupOpen} onOpenChange={(next) => !busy && setSetupOpen(next)}>
        <DialogContent className="sm:max-w-lg" onClick={(e) => e.stopPropagation()}>
          <DialogHeader>
            ...
            <DialogDescription>{setupReason || "Depois desta configuração, o botão Nota usará diretamente estas preferências."}</DialogDescription>
          </DialogHeader>
          <div className="space-y-5 py-1">
            <div className="rounded-xl border border-border bg-muted/30 px-3 py-2.5 text-xs text-muted-foreground">Esta etapa só aparece na primeira utilização da conta neste dispositivo ou quando a impressora configurada deixa de estar disponível.</div>
            <div className="space-y-2">
              ...
              <div className="grid gap-2">
                ...
                <button type="button" onClick={() => setSettings((s) => ({ ...s, transport: "bluetooth" }))} className={"w-full rounded-xl border p-3 text-left transition " + (settings.transport === "bluetooth" ? "border-primary bg-primary/5" : "border-border hover:bg-muted/60")}>
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">Impressão direta para térmicas compatíveis com Web Bluetooth.</div>
                </button>
              </div>
            </div>

            {desktop && settings.transport === "system" && <DesktopPrinterSetup value={settings.printerName} onChange={(printerName) => setSettings((s) => ({ ...s, printerName }))} disabled={busy} />}
            ...
            <div className="space-y-2">
              ...
              {(settings.paperId === "custom" || settings.paperId === "continuous") && (
                <div className={`grid gap-3 pt-1 ${settings.paperId === "custom" ? "grid-cols-2" : "grid-cols-1"}`}>
                  ...
                  {settings.paperId === "custom" && <label className="space-y-1 text-xs text-muted-foreground"><span>Altura (mm)</span><input type="number" min={60} max={500} step={0.1} value={settings.customHeightMm} onChange={(e) => setSettings((s) => ({ ...s, customHeightMm: Number(e.target.value) }))} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring" /></label>}
                </div>
              )}
              <div className="text-xs text-muted-foreground">{paper.continuous ? `Papel contínuo: ${paper.widthMm} mm de largura · altura automática.` : `Área da nota: ${paper.widthMm} × ${paper.heightMm} mm.`}</div>
            </div>

            {settings.transport === "bluetooth" && <div className="space-y-2"><div className="text-sm font-medium text-foreground">Resolução térmica</div><div className="flex gap-2">{[203, 300].map((dpi) => <button key={dpi} type="button" onClick={() => setSettings((s) => ({ ...s, dpi: dpi as 203 | 300 }))} className={"rounded-lg border px-3 py-2 text-xs font-medium transition " + (settings.dpi === dpi ? "border-primary bg-primary/5 text-primary" : "border-border text-muted-foreground hover:bg-muted/60")}>{dpi} DPI</button>)}</div></div>}
          </div>
          <DialogFooter>
            <Button variant="ghost" type="button" disabled={busy} onClick={() => setSetupOpen(false)}>Agora não</Button>
            <Button type="button" disabled={busy || (settings.transport === "bluetooth" && !bluetoothReady) || (desktop && settings.transport === "system" && !settings.printerName?.trim())} onClick={saveAndPrintFromSetup}><Printer className="h-4 w-4 mr-2" />{busy ? "Preparando…" : "Salvar e imprimir"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
...
function isPrinterAvailabilityError(error: unknown) {
  ...
  return ["notfounderror", "notreadableerror", "networkerror", "securityerror", "bluetooth", "gatt", "device", "dispositivo", "impressora", "printer", "usb"].some((token) => textValue.includes(token));
}
