...
if (currentVersion !== "0.6.6") {
  throw new Error(`Unexpected DentalFlow Desktop release: ${currentVersion || "missing"}; expected 0.6.6.`);
}
...
const adapted = source
  ...
  .replaceAll('Notification reconciliation must remain low-latency in 0.3.5.', 'Notification reconciliation must remain low-latency in 0.6.6.')
...
const soundBridge = fs.readFileSync("src/components/DesktopNotificationSoundBridge.tsx", "utf8");
const nativeNotifications = fs.readFileSync("src-tauri/src/notifications.rs", "utf8");
...
if (!soundBridge.includes("dentalflow:realtime-notification") || !soundBridge.includes("playDesktopNotificationSound")) {
  throw new Error("Desktop 0.6.6 must play one native sound for every canonical realtime notification.");
}
if (!nativeNotifications.includes("SystemNotification") || !nativeNotifications.includes("MessageBeep")) {
  throw new Error("Desktop 0.6.6 must use the Windows notification sound scheme with a native fallback.");
}
if (!privacyMigration.includes("case_activity_select_v064") || !privacyMigration.includes("case_cadista_assignment_started_at")) {
  throw new Error("Desktop 0.6.6 must version the CAD chat assignment privacy boundary.");
}
